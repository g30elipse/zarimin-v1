    ...
    <section ref={triggerRef} className="overflow-hidden bg-[#ecebe9]">
      <div className="h-screen flex items-center">
        ...
        <div className="absolute top-20 left-20 z-10">
          <h2 className="text-[10vw] font-black leading-none uppercase tracking-tighter opacity-10">
            Portfolio
          </h2>
          <p className="text-sm font-bold tracking-[0.4em] uppercase mt-[-2rem] ml-2">
            Selected Works — 01/04
          </p>
        </div>

        {/* Horizontal Container */}
        <div ref={scrollRef} className="flex gap-20 px-[20vw] items-center">
          {works.map((work, i) => (
            <div key={i} className="work-card w-[400px] md:w-[600px] flex-shrink-0 group cursor-none">
              <div className="relative aspect-[16/10] overflow-hidden bg-zinc-200">
                ...
                />
                <div className="absolute top-4 right-4 text-[10px] font-mono bg-black text-white px-2 py-1">
                  CASE_{i + 1}
                </div>
              </div>
              <div className="mt-6 flex justify-between items-end border-b border-black pb-4">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">{work.category}</p>
                  <h3 className="text-3xl font-serif italic">{work.title}</h3>
                </div>
